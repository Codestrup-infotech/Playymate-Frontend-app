...
const s3Config = {
  region: process.env.WASABI_SOCIAL_REGION || process.env.WASABI_REGION || "ap-south-1",
  endpoint: process.env.WASABI_SOCIAL_ENDPOINT || process.env.WASABI_ENDPOINT || "https://s3.ap-south-1.wasabisys.com",
  credentials: {
...
const BUCKET_NAME = process.env.WASABI_SOCIAL_BUCKET || process.env.WASABI_BUCKET || "playymate";
...
export async function POST(request) {
  try {
    ...
    // Construct full URLs
    const wasabiDirectUrl = `${s3Config.endpoint}/${BUCKET_NAME}/${fullKey}`;
