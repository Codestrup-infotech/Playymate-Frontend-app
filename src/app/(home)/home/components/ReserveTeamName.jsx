...
export default function ReserveTeamName({ isOpen, onClose, teamName, onSuccess, onSkip, onContinue }) {
  ...
  const handleReserve = async () => {
    ...
    try {
      ...
      const idempotencyKey = `reserve_${nameToReserve.replace(/\s+/g, '_')}_${Date.now()}`
      
      const result = await reserveTeamName({
        name: nameToReserve,
      ...
      setReservationResult(result.data || result)
      setStep("success")
      ...
      setError(err.message || "Failed to reserve name")
      setStep("error")
    } finally {
      setLoading(false)
    }
