...
//           // Both KYC screens disabled → Skip KYC entirely, go to physical
//           router.push('/onboarding/physical');
//           return;
//         }
        
//         if (aadhaarEnabled || livenessEnabled) {
//           // At least one KYC screen enabled → Go to KYC page
...
function ActivityIntentContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
...
function ActivityIntentLoading() {
  return (
    <div className="bg-black">
      <div className="w-[390px] py-8 text-white mx-auto">
        <div className="flex gap-6 mb-6 pt-6">
        ...
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="w-full h-16 bg-gray-800 rounded-xl animate-pulse"></div>
          ))}
        </div>
      </div>
    </div>
...
export default function ActivityIntentPage() {
  return (
    <Suspense fallback={<ActivityIntentLoading />}>
      <ActivityIntentContent />
    </Suspense>
  );
}
