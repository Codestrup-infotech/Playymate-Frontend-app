...
//       // ✅ /subscriptions/me: { status, data: { plan_name, status, price, start_date,
//       //    end_date, billing_cycle:{value,unit}, coins_remaining, features[], next_renewal, subscription_id } }
//       if (sj?.status === "success" && sj?.data) setSubscription(sj.data);
//       else throw new Error(sj?.message || "No active subscription found.");

//       // ✅ /coins/balance: { data: { gold_coins: { balance, status, expires_at, days_remaining } } }
//       if (cj?.status === "success") setCoins(cj.data?.gold_coins || null);

//       // ✅ /diamonds/balance: { data: { balance, status } }
//       if (dj?.status === "success") setDiamonds(dj.data || null);

//       // ✅ /coins/history: { data: { transactions: [{ _id,type,amount,source,source_reference,
//       //    balance_before,balance_after,expires_at,status,created_at,coin_type }] } }
...
//         {/* Coin History — from GET /coins/history */}
